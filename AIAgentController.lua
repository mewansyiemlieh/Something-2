...
-- addLog is reassigned to the real GUI implementation once the log frame is built.
-- The no-op default prevents "attempt to call nil" if any event fires before GUI loads.
local addLog = function() end

-- Safe player acquisition — works even if LocalPlayer isn't ready yet
local player
do
    local ok, lp = pcall(function() return Players.LocalPlayer end)
    player = (ok and lp) or nil
    if not player then
        -- Wait for LocalPlayer to become available (rare edge case in some executors)
        local waited = 0
        repeat task.wait(0.05); waited = waited + 0.05
            local ok2, lp2 = pcall(function() return Players.LocalPlayer end)
            if ok2 and lp2 then player = lp2; break end
        until waited > 5
    end
    if not player then error("AIAgent: Could not obtain LocalPlayer") end
end
...
-- All executor globals are wrapped in pcall so a missing
-- global never throws "attempt to index nil" on line 1.
--====================================================
...
do
    ...
    local syn_    = _safeGlobal("syn")
    local http_   = _safeGlobal("http")
    local flux_   = _safeGlobal("fluxus")
    if syn_ and syn_.request then
        httpRequest = syn_.request
    elseif http_ and http_.request then
        httpRequest = http_.request
    elseif flux_ and flux_.request then
        httpRequest = flux_.request
    else
        local ok, fn = pcall(function() return getfenv()["request"] end)
        if ok and type(fn) == "function" then httpRequest = fn end
    end
end
...
local function askQnA(prompt, onAnswer, onError)
    task.spawn(function()
        ...
        local messages = {}
        local stateStr = "unavailable"
        pcall(function() stateStr = getGameState() end)
        local stateCtx = { role = "user",      content = "Game context: " .. stateStr }
...
local function dexSearch(searchRoot, predicate)
    local results = {}
    local ok, desc = pcall(function() return searchRoot:GetDescendants() end)
...
-- GAME STATE
--====================================================
local getGameState  -- forward declaration so askQnA / askAI always call the patched version
...
-- Forward declarations: getChatLog and callAPI are defined later in the file,
-- but chatbotReply (below) calls both. Declaring them upfront prevents
-- "attempt to call nil value" if a chat event fires before full load.
local getChatLog  -- assigned further down in the Chat Listener section
local callAPI     -- assigned further down in the Low-Level API Call section
...
local function moveToPos(targetPos, speed)
    ...
    if refreshRefs() and humanoid then
...
local function executeAction(act)
    ...
    if aType == "stop" then
        stopFlag = true
        if humanoid then humanoid:MoveTo(root.Position) end -- cancel current MoveTo
        task.wait(0.1)
        if refreshRefs() and humanoid then humanoid.WalkSpeed = 16 end
    ...
    elseif aType == "jump" then
        ...
        for i = 1, count do
            if not refreshRefs() or not humanoid then break end
            pcall(function() humanoid.Jump = true end)
            -- Wait until airborne
            local t0 = tick()
            repeat task.wait(0.05) until
                (not humanoid or humanoid.FloorMaterial == Enum.Material.Air) or (tick()-t0 > 0.4)
            -- Wait until landed again
            t0 = tick()
            repeat task.wait(0.05) until
                (not humanoid or humanoid.FloorMaterial ~= Enum.Material.Air) or (tick()-t0 > 2)
...
--====================================================
callAPI = function(systemPrompt, userMsg, onDone, onError)
...
local function askAI(prompt, onResult, onError, onTranslated)
    task.spawn(function()
        ...
        pcall(function() state = getGameState() end)
...
local function startESPLoop()
    ...
    espUpdateConn = RunService.Heartbeat:Connect(function()
        ...
        for target, obj in pairs(espObjects) do
            ...
            local tgtRoot = char and char:FindFirstChild("HumanoidRootPart")

            -- Hide if no character or exceeds max distance
            local dist = tgtRoot and (tgtRoot.Position - myPos).Magnitude or math.huge
...
local function dexSelectInst(inst)
    ...
    for idx, prop in ipairs(props) do
        ...
        vBox.FocusLost:Connect(function(enter)
            if enter then
                -- Directly set the property via pcall using the captured inst reference
                local directCode = string.format(
                    "local ok,e = pcall(function() game:GetService('Players').LocalPlayer.Character and true\n"
                    .. "local ok2,e2 = pcall(function() (%s).%s = %s end)\n"
                    .. "return ok2 and 'set ok' or tostring(e2) end)\n"
                    .. "return ok and 'set ok' or tostring(e)",
                    inst:GetFullName(), propName, vBox.Text)
                local ok2, res = dexExec(directCode)
                addLog("prop " .. propName .. ": " .. tostring(res), ok2 and C(180,255,200) or C(255,120,120))
            end
        end)
    end
